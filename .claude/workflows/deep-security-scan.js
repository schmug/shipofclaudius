...
Produce:
1. Create an output dir OUTSIDE the target repo's working tree: run \`mkdir -p "${OUTPUT_ROOT}/$(date -u +%Y%m%dT%H%M%SZ)-deep"\` and use it (capture the absolute path; no Date.now in scripts — stamp via date -u). ${OUTPUT_RULE}
