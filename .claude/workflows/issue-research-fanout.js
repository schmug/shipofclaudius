...
// {number, updatedAt, headSha, SPINE_VERSION}. On re-run we skip an entry iff it is
// present, done, its issue's `updatedAt` is unchanged, it was written by THIS spine
// version, AND the repo TREE has not moved under its own files[] footprint since it was
// computed (#223: a verdict is a claim about REPO STATE, not just about issue text — an
// unchanged `updatedAt` alone is not proof the verdict still holds).
//
...
//     the repo's current HEAD sha in ONE batched call, so the skip decision happens
//     BEFORE the expensive chain.
...
const CKPT_META_PROMPT = (nums) =>
  ...
  `Return { items: [{ number, updatedAt }, ...], headSha } covering EVERY requested number. Read-only: run no mutating command; do NOT edit, comment, relabel, push, merge, or open anything.`
...
const CKPT_TREECHECK_PROMPT = (shas) =>
  ...
  `Return { diffs: [{ sha, changedFiles, unresolved }, ...] } covering EVERY sha above exactly once. Read-only: run no mutating command; do NOT edit, comment, relabel, push, merge, or open anything.`
...
if (SHAS_TO_CHECK.size) {
  ...
  const treeRes = await agent(CKPT_TREECHECK_PROMPT(shaList), { label: 'ckpt-treecheck', phase: 'Research', agentType: READONLY_AGENT, schema: CKPT_TREECHECK_SCHEMA, effort: 'low' })
