...
const TRIAGE_SCHEMA = {
  ...
  properties: {
    ...
    decision_question: { type: 'string', description: 'For DECISION: the single crisp question the human must answer. If the issue is already a well-formed needs-decision brief, reuse its own question verbatim — do not re-derive it. Empty otherwise.' },
    decision_options: { type: 'array', items: { type: 'string' }, description: 'For DECISION: 2-4 concrete options, recommended first. If the issue is already a well-formed needs-decision brief, reuse its own options verbatim — do not re-derive them. Empty otherwise.' },
...
3. Classify into exactly one bucket:
   ...
   - DECISION: implementing requires a genuine human product/architecture choice with NO sensible default. Put the question in decision_question + 2-4 options (recommended first) in decision_options. TRUST AN EXISTING BRIEF: if the fenced data shows the issue already carries the \`needs-decision\` label AND its body already states a clear question with 2-4 options (i.e. it is a filed decision brief), do NOT re-derive the framing — classify DECISION and copy the brief's own question into decision_question and its own options into decision_options verbatim. The only allowed override for such a brief is DONE or GREEN, and only when current repo state has actually resolved the question (the next sentence's carve-out); never silently reclassify a well-formed \`needs-decision\` brief as RESEARCH. A decision brief is never \`needs-you\` material — \`needs-you\` means "stop and do not act", which contradicts a brief's own contract of finishing everything that doesn't depend on the answer — never suggest or imply needs-you treatment for one. If the issue's own "decision needed" section is already resolved by current repo state, do NOT mark DECISION — mark DONE or GREEN.
