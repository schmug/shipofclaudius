...
`report.html` is rendered by **deterministic code in the workflow script**, not authored by a model — every attacker-derived finding field is HTML-escaped and each finding's `file:line` is checked against the diff (a line outside every hunk is kept but marked "not in diff", never dropped). The result carries both `report_html` and `report_md` as full text, plus `report_dir` (a UTC-stamped directory the script cannot create itself — it has no clock or filesystem access). After a run, **persist both files yourself** with the Write tool: `report_dir/report.html` and `report_dir/report.md`.
