...
Run the `stacked-impl-lanes` dynamic workflow bundled with this plugin. `Workflow({ scriptPath })` only accepts a path already under the session's working directory (or an added directory) — a plugin-cache path is refused even after being `Read` in-session (see `CLAUDE.md` "Wrapper shape"; reproduced in [#213](https://github.com/schmug/shipofclaudius/issues/213)). So: `Read` `${CLAUDE_PLUGIN_ROOT}/.claude/workflows/stacked-impl-lanes.js`, then call the Workflow tool with its exact contents as `script`:
...
Pass a caller-specific taxonomy when the repo has its own recurring defect shapes (again, `script` — not `scriptPath` — carrying the file's exact contents):

```
Workflow({ script: "<the file's exact contents>", args: {
  mode: "sequential",
