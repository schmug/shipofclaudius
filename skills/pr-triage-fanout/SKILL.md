...
Fill `args` from the user's request. Common args: `numbers` (subset; omit to auto-gather), `repo`, `author`, `notes`. For the full, current argument list, read the header comment / `meta` block in `${CLAUDE_PLUGIN_ROOT}/.claude/workflows/pr-triage-fanout.js`, or the repo README "Arguments" table. READ-ONLY; triages only the resolved author's PRs. A read-scoped `gh` token is still recommended (fine-grained PAT limited to read, or a wrapper that rejects mutating subcommands) — the workflow now also verifies this itself: a preflight check reads the ambient token's scope and fails fast with a clear error if it is write-capable, rather than only documenting the requirement (see README "Required setup: a read-scoped gh token").
