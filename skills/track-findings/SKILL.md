...
Run the `track-findings` dynamic workflow bundled with this plugin. `Workflow({ scriptPath })` only accepts a path already under the session's working directory (or an added directory) — a plugin-cache path is refused even after being `Read` in-session (see `CLAUDE.md` "Wrapper shape"; reproduced in [#213](https://github.com/schmug/shipofclaudius/issues/213)). So: `Read` `${CLAUDE_PLUGIN_ROOT}/.claude/workflows/track-findings.js`, then call the Workflow tool with its exact contents as `script`:
...
Untrusted finding text (title, evidence, attacker_story, fix, source/sink) reaches the write agent only inside a nonce fence behind an anti-injection preamble; the bundle-load and repo-context steps run under a read-only `agentType` (`readonlyAgent`, default `Explore`). The file agent deliberately keeps its write tools — filing is the intended behaviour.
