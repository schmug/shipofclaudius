...
**One exception to "no application".** The repo also ships a small MCP stdio server at [`packages/vent-server/`](packages/vent-server/) — Node built-ins only, like everything else here — registered by the root [`.mcp.json`](.mcp.json) and launched by the host in every session where the plugin is installed. Three consequences worth knowing before you touch it:
