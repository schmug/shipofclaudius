...
**`factory-intake` + `factory-build` operate on a *different* repository than the session's.** The intake skill creates the project repo and every build agent clones it under `${TMPDIR:-/tmp}/factory/<slug>/<key>/`; nothing in that pipeline touches the session's own checkout, and the build agent deliberately declares **no** `isolation: 'worktree'` (a worktree of *this* repo would be the wrong tree). Four invariants ride on it, all sim- or test-enforced. Every `wrangler deploy` in a build prompt carries `--config wrangler.preview.<key>.jsonc`; the production `wrangler.jsonc` deploys only from the skill's promote phase after `merge-pr-with-gate`, and names its config explicitly so wrangler's own discovery never picks one for you. The build agent is untrusted, so the session scores in a clone **it makes itself** (`score-<key>`, never the build agent's directory) and runs the tamper guard first: a `git diff --quiet "$SCAFFOLD_SHA" HEAD` over `scripts/`, the dependency manifests, `.npmrc`, `.env*`/`.dev.vars*`, `.gitignore`, every wrangler config and `.github/` — every pathspec carrying `:(icase)`, because git pathspecs are case-sensitive even on a case-insensitive volume — followed by the checkout checks (non-ASCII path, case-folding collision, unclean tree, and `^120000 ` for a symlink, which would redirect a script's writes or the deploy's asset upload). Read that block by its `TAMPERED` output and its closing `GUARD_DONE`, never by its exit code. The Access service token is read from `process.env` by exactly two scaffold scripts (`scripts/smoke.mjs`, `scripts/critic.mjs`) and its variable names appear in the build prompt only inside the do-not-echo sentence. And no personal domain or account appears in the skill, the workflow, or the scaffold — the plugin is public, so hosting arrives as `FACTORY_PREVIEW_DOMAIN` / `FACTORY_PROD_DOMAIN` and `args.previewDomain`, and the owner defaults to `gh api user --jq .login`. The scaffold lives at `skills/factory-intake/scaffold/`, **not** under `references/`, because `plugin-integrity` resolves every `references/<name>` token in a process skill with `readFile` and a directory fails it. The per-invariant controls and their residual risks are `skills/factory-intake/THREAT_MODEL.md` — read it before changing the scaffold, the guard, or `critic.mjs`.
...
- **Wrapper shape.** Each `SKILL.md` frontmatter must have `name: <name>` matching the workflow and a non-empty `description`. `Workflow({ scriptPath })` only accepts a path already under the session's working directory (or a directory explicitly added to it) or a path the tool itself returned earlier in the session — **never an arbitrary on-disk path**, and a plugin-cache path is refused even after being `Read` in-session first (reproduced and scoped in #213; the contrary claim in `docs/specs/2026-06-21-workflow-plugin-packaging-design.md` §3 is corrected there). So the body must instead instruct: **`Read` `${CLAUDE_PLUGIN_ROOT}/.claude/workflows/<name>.js`, then call `Workflow({ script: <that file's exact contents>, ... })`** — referencing its own bundled script by content, not by `scriptPath`. This is still zero-copy: nothing is duplicated at rest, the content is re-read from the single canonical file on every invocation.
