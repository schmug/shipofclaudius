...
const REPORT_OK_DIFF = {
  output_dir: '/tmp/x/shipofclaudius-scans/T-diff',
  report_html_path: '/tmp/x/shipofclaudius-scans/T-diff/report.html',
  report_md: '# Change-Scoped Security Review — merged md',
  html_written: true,
}
...
test('output: a PUBLIC target triggers a disclosure warning naming the private intake', async () => {
  const map = { report: { ...REPORT_OK_DIFF, target_visibility: 'PUBLIC' } }
  const { result, calls } = await runScript({ args: { target: '/tmp/fake' }, map })
...
test('output: unknown visibility fails CLOSED (still warns)', async () => {
  const map = { report: { ...REPORT_OK_DIFF, target_visibility: '' } }
  const { calls } = await runScript({ args: { target: '/tmp/fake' }, map })
...
test('output: a PRIVATE target does not warn', async () => {
  const map = { report: { ...REPORT_OK_DIFF, target_visibility: 'PRIVATE' } }
  const { result, calls } = await runScript({ args: { target: '/tmp/fake' }, map })
