...
test('turns: a CI monitor wake event is not a human turn even though origin.kind reads human', () => {
  // Claude Code writes these with `origin.kind: "human"`, so the structural gate above does
  // not catch them — only the prefix gate can, and until now it did not name this tag.
  const text = [
    trLine({ promptId: 'p1', uuid: 'u1', message: { role: 'user', content: 'fix the timeout' } }),
    trLine({ promptId: 'p2', uuid: 'u2', message: { role: 'user', content: '<ci-monitor-event>CI failed on PR #42</ci-monitor-event>' } }),
  ].join('\n')
  assert.deepEqual(humanTurns(text).map((t) => t.prompt_id), ['p1'])
})

// A generic canary, independent of SYNTHETIC_PREFIX_RE's own alternation: any surviving turn
// whose text opens with a tag shape is machine-generated by definition, whatever the tag is
// named. Written this way so the next prefix Claude Code introduces fails this test on its
// own, rather than requiring someone to remember to add a matching assertion for it.
const LEADING_TAG_RE = /^\s*<[A-Za-z][\w-]*(?:[ >])/

test('turns: the surviving-turn census contains no leading-tag shapes, named or not yet imagined', () => {
  const text = [
    trLine({ promptId: 'p1', uuid: 'u1', message: { role: 'user', content: 'fix the timeout' } }),
    trLine({ promptId: 'p2', uuid: 'u2', message: { role: 'user', content: '<ci-monitor-event>CI failed</ci-monitor-event>' } }),
    trLine({ promptId: 'p3', uuid: 'u3', message: { role: 'user', content: '<command-name>ship</command-name>' } }),
    trLine({ promptId: 'p4', uuid: 'u4', message: { role: 'user', content: '<system-reminder>context</system-reminder>' } }),
    trLine({ promptId: 'p5', uuid: 'u5', message: { role: 'user', content: 'now run the tests' } }),
  ].join('\n')
  const survivors = humanTurns(text)
  assert.ok(survivors.length > 0, 'a real human turn should still survive')
  for (const t of survivors) {
    assert.equal(LEADING_TAG_RE.test(t.text), false, `"${t.text}" looks machine-generated but survived humanTurns`)
  }
})

test('join: a CI-monitor-event turn drops out of joinLabels rather than entering the labelled set', () => {
  const text = [
    trLine({ promptId: 'p1', uuid: 'u1', message: { role: 'user', content: 'fix the timeout' } }),
    trLine({ promptId: 'p2', uuid: 'u2', message: { role: 'user', content: '<ci-monitor-event>CI failed on PR #42</ci-monitor-event>' } }),
  ...
  const records = [
    { prompt_id: 'p1', grounded: 1, unresolved: 0, ambiguous: 0, indeterminate: 0, log_length_baseline: 1 },
    { prompt_id: 'p2', grounded: 1, unresolved: 0, ambiguous: 0, indeterminate: 0, log_length_baseline: 1 },
  ]
  const { rows, unmatched } = joinLabels(records, pairs)
  assert.deepEqual(rows.map((r) => r.prompt_id), ['p1'], 'the CI event never enters the labelled set')
  assert.equal(unmatched, 1, "the CI event's own log record can never join, permanently")
})
