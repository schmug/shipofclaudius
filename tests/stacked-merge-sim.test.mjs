...
test('meta advertises the post-merge base verification that the land prompt implements', async () => {
  ...
  const descMatch = /description:\s*'([^']*)'/.exec(metaSrc)
  assert.ok(descMatch, 'meta.description is a single-quoted literal string')
  assert.ok(/\bmissing\b/i.test(descMatch[1]), 'meta.description itself names the missing/unverified stop condition, not just red (#125)')
  const { calls } = await runScript({ args: { prs: [1] } })
