...
// point at.
import { readFile, mkdtemp, realpath, mkdir, writeFile, readdir } from 'node:fs/promises'
...
test('file-concerns: drains by rotate, never by truncate-in-place', async () => {
  ...
  assert.match(md, /\bmv\s+"?(~|\$HOME)\/\.claude\/concerns-spool\.jsonl/, 'rotates the live spool with mv')
...
// A `~` inside double quotes is not expanded, so `mv ... "~/.claude/..."` targets a literal
// `./~/.claude/` directory and fails "No such file or directory". Run the shipped line for real.
test('file-concerns: the rotate command really rotates the spool', async () => {
  const md = await read('skills/file-concerns/SKILL.md')
  const rotate = md.split('\n').find((l) => /^mv\s.*concerns-spool\.jsonl/.test(l))
  assert.ok(rotate, 'the skill carries a one-line mv rotate command')
  const home = await realpath(await mkdtemp(join(tmpdir(), 'file-concerns-rotate-')))
  await mkdir(join(home, '.claude'))
  await writeFile(join(home, '.claude', 'concerns-spool.jsonl'), '{"x":1}\n')
  execFileSync('sh', ['-c', rotate], { cwd: home, env: { ...process.env, HOME: home }, stdio: 'pipe' })
  const left = await readdir(join(home, '.claude'))
  assert.ok(!left.includes('concerns-spool.jsonl'), 'the live spool is gone after the rotate')
  const rotated = left.filter((f) => /^concerns-spool\.\d{8}T\d{6}Z\.jsonl$/.test(f))
  assert.equal(rotated.length, 1, `exactly one timestamped spool, got: ${left.join(', ')}`)
  assert.equal(await readFile(join(home, '.claude', rotated[0]), 'utf8'), '{"x":1}\n', 'contents ride along')
})
