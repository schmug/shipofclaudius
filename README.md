...
| [`routine-anti-noise.js`](.claude/workflows/routine-anti-noise.js) | `routine-anti-noise` | Read-only anti-noise gate for the fleet routines: given a PR or issue number, a read-only relay resolves issue-vs-PR and its labels/state (+ a PR's linked issue), then a **skip-label** decision runs in code (`needs-you`/`needs-decision`/`awaiting-human`/`impl-blocked`/`routine-pause`/`wontfix`/`duplicate` on the target or its linked issue) and, if not skipped, a read-only agent scans the last ~10 comments for an existing `_Generated by Claude Code_`-signed comment already conveying the same intent with no owner reply since. Returns a `{skip, reason}` / `{duplicateComment}` decision object — never posts, labels, or merges. |
...
| `routine-anti-noise` | `number` (**required** — the issue or PR to gate), `repo?`, `type?` (`"issue"`\|`"pr"` hint; auto-detected if omitted), `skipLabels?` (override the default skip-label list), `intent?` (the comment the caller is about to post, for a targeted duplicate check), `readonlyAgent?` | No args required beyond `number`. Labels/state are extracted as structured metadata and gated **in code**; only comment bodies are nonce-fenced for the read-only duplicate-comment agent (see **Security model**). Returns `{skip, reason}` on a skip-label hit, else `{skip:false, duplicateComment, duplicateReason, duplicateExcerpt}`. |
...
2. **Every subagent runs through a read-only `agentType`.** Default is the built-in **`Explore`** (no `Edit` / `Write` / `NotebookEdit` / sub-`Agent`), so tool access is restricted by the runtime regardless of what the fenced text says. Override with `args.readonlyAgent: "<your-agent>"` to use a stricter custom read-only agent. (The three **write** workflows are the exception — their actors **must** keep write tools: `stacked-impl-lanes`' impl agent pushes and opens PRs, `stacked-merge-walk`' land/cleanup actors rebase, force-push-with-lease, and merge, and `fix-finding`' fix agent commits a regression test + minimal fix and opens a draft PR. So `readonlyAgent` scopes only their *read-only* relays/gates — `stacked-impl-lanes`' issue-text relays, `stacked-merge-walk`' PR-text relays **and** its read-only verify gate, and `fix-finding`' read-only triage agent — never the write actor. Their mitigation is the fence + preamble, plus `stacked-impl-lanes`' `security-hardening-reviewer` gate on invariant lanes, `stacked-merge-walk`' read-only verify gate + the deliberate choice to keep untrusted PR text out of the land actor entirely, and `fix-finding`' adversarial `security-hardening-reviewer` gate that refuses to bless a fix which weakens a control. `security-diff-scan` is the same shape: its resolve/discovery/validation agents are read-only; only its final **report** agent keeps write tools to create `report.html`, and that agent sees only already-validated findings — never the raw untrusted diff/PR text unescaped.)
...
`routine-anti-noise` reads the least of the group: the target's (and, for a PR, its linked issue's) **labels/state** are structured, operational metadata extracted directly by its read-only RESOLVE relay and used **in code** for the skip-label decision — never handed to a reasoning agent. Only the free-text **comment bodies** it scans for an existing duplicate blocker comment are nonce-fenced as `UNTRUSTED DATA` for its read-only GATE agent, and that agent only returns a `{duplicateFound, reason}` verdict — it never comments, labels, or merges itself.
...
The three **write** workflows are the exception — do **not** run them under the read-only token; rely on their fence + preamble (and gates) instead:
...
```bash
npm test          # runs all fifteen simulator suites plus the plugin-integrity check
# or individually:
...
node tests/routine-anti-noise-sim.test.mjs
node tests/plugin-integrity.test.mjs
```

Requires Node ≥ 18 (developed on Node 22). Current status: **466 passing** (12 + 65 + 49 + 31 + 28 + 34 + 19 + 29 + 18 + 62 + 26 + 24 + 20 + 23 + 22 + 4), 0 failing.
...
└── tests/
    ...
    ├── routine-anti-noise-sim.test.mjs # simulates routine-anti-noise.js
