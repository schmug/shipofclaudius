...
| `stacked-impl-lanes` | `lanes` (required: `[{ key, branch, issues, invariant, brief }]`), `mode?` (`parallel` \| `sequential`, default `parallel`), `base?` (default `main`), `repo?`, `adversarialReview?` (`opened` \| `invariant` \| `off`, default `opened`), `defectClasses?` (strings or `{key,title,focus}`; replaces the generic defaults), `batchSize?` (parallel-lane wave size, **default 4** — a lane is heavier than one triage item), `readonlyAgent?` | **Writes** — opens review-only PRs. `readonlyAgent` scopes only its issue-text relays and read-only Review critics, not the impl agent. A lane any critic **gates** never becomes the branch base its dependents stack onto (a `BLOCKED` lane still doesn't break the chain — the base just doesn't advance). |
