...
| `stacked-merge-walk` | `prs` (required, base-first: `[n,…]` or `[{ pr, branch }]`; also accepts `branches: [name,…]` or `lanes: [{ key, branch }]` from `stacked-impl-lanes`), `base?` (default `main`), `repo?`, `readonlyAgent?`, `postMergeVerify?` (default `true` — re-runs the gate on the base itself right after each squash-merge; `false` is the escape hatch for a slow target suite) | **Writes** — rebases/merges the stack. `readonlyAgent` scopes only its read-only PR-text relays + the read-only verify gate, not the write land/cleanup actors. A PR that can't land stops the walk; the landed prefix is reported. A base that goes red on the post-merge run cannot be repaired here (no push to base) and also stops the walk. |
...
Requires Node ≥ 18 (developed on Node 22). Current status: **645 passing** (12 + 65 + 49 + 31 + 28 + 34 + 19 + 44 + 18 + 62 + 26 + 24 + 20 + 23 + 23 + 26 + 59 + 44 + 33 + 5), 0 failing.
